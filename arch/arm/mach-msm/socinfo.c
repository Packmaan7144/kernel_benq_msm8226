...
static oem_hw_info_type *oem_hw_info = NULL;
                   ...
                   char *buf)
{
	...
	if (!vendor1_data) {
		pr_err("%s: No vendor1_data found!\n", __func__);
		return 0;
	}
...
static ssize_t
socinfo_show_oem_hw_info_board_id(struct sys_device *dev,
			struct sysdev_attribute *attr,
			char *buf)
{
	if (!oem_hw_info) {
		pr_err("%s: No oem_hw_info found!\n", __func__);
		return 0;
	}

	return snprintf(buf, PAGE_SIZE, "0x%x\n", oem_hw_info->board_id);
}

static ssize_t
socinfo_show_oem_hw_info_variant_id(struct sys_device *dev,
			struct sysdev_attribute *attr,
			char *buf)
{
	if (!oem_hw_info) {
	...
	return snprintf(buf, PAGE_SIZE, "0x%x\n", oem_hw_info->variant_id);
}

static ssize_t
socinfo_show_oem_hw_info_ddr_vendor_id(struct sys_device *dev,
			struct sysdev_attribute *attr,
			char *buf)
{
	if (!oem_hw_info) {
	...
	return snprintf(buf, PAGE_SIZE, "%d\n", oem_hw_info->ddr_vendor_id);
}

static ssize_t
socinfo_show_oem_hw_info_ddr_size(struct sys_device *dev,
			struct sysdev_attribute *attr,
			char *buf)
{
	if (!oem_hw_info) {
	...
	return snprintf(buf, PAGE_SIZE, "%d\n", oem_hw_info->ddr_size);
}

static ssize_t
socinfo_show_oem_hw_info_cpu_feature_id(struct sys_device *dev,
			struct sysdev_attribute *attr,
			char *buf)
{
	if (!oem_hw_info) {
	...
	return snprintf(buf, PAGE_SIZE, "0x%x\n", oem_hw_info->cpu_feature_id);
}

static ssize_t
socinfo_show_oem_hw_info_cpu_hw_revision(struct sys_device *dev,
			struct sysdev_attribute *attr,
			char *buf)
{
	if (!oem_hw_info) {
...
static struct sysdev_attribute oem_hw_info_files[] = {
	_SYSDEV_ATTR(board_id, 0444, socinfo_show_oem_hw_info_board_id, NULL),
	_SYSDEV_ATTR(variant_id, 0444, socinfo_show_oem_hw_info_variant_id, NULL),
	_SYSDEV_ATTR(ddr_vendor_id, 0444, socinfo_show_oem_hw_info_ddr_vendor_id, NULL),
	_SYSDEV_ATTR(ddr_size, 0444, socinfo_show_oem_hw_info_ddr_size, NULL),
	_SYSDEV_ATTR(feature_id, 0444, socinfo_show_oem_hw_info_cpu_feature_id, NULL),
	_SYSDEV_ATTR(hw_revision, 0444, socinfo_show_oem_hw_info_cpu_hw_revision, NULL),
	_SYSDEV_ATTR(imei, 0444, socinfo_show_imei, NULL), 
};
...
static int __init socinfo_init_sysdev(void)
{
	...
	socinfo_create_files(&soc_sys_device, oem_hw_info_files,
				ARRAY_SIZE(oem_hw_info_files));
...
{
	smem_vendor_id2_bl_data* id2_bl_data = NULL;
	id2_bl_data = (smem_vendor_id2_bl_data *)smem_alloc(SMEM_ID_VENDOR2, sizeof(smem_vendor_id2_bl_data));
	oem_hw_info = &(id2_bl_data->hw_info);
}
        ...
        if (!vendor1_data)
                pr_err("Alloc share memory SMEM_ID_VENDOR1 faili[%d]!\n", sizeof(smem_vendor_id1_apps_data));
...
board_id_type socinfo_get_oem_board_id(void)
{
	return oem_hw_info->board_id;
}



variant_id_type socinfo_get_oem_variant_id(void)
{
	return oem_hw_info->variant_id;
}



uint32_t socinfo_get_factory_cable(void)
{
	return oem_hw_info->facable;
}
