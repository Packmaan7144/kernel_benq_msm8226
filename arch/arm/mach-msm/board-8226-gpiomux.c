...
static struct msm_gpiomux_config msm_sensor_configs_evt0[] __initdata = {
#if 1
	{ 
		.gpio = 36, 
		.settings = {
...
static struct msm_gpiomux_config msm_sensor_configs_for_workaround_evt1_2[] __initdata = {
	{
...
static struct msm_gpiomux_config msm_pn544_nfc_configs[] __initdata = {
	{
		.gpio = PN544_HOST_INT_GPIO, 
		.settings = {
...
static struct gpiomux_setting factory_cable_detect_cfg = {
	.func = GPIOMUX_FUNC_GPIO,
	.drv = GPIOMUX_DRV_2MA,
	.pull = GPIOMUX_PULL_DOWN,
	.dir = GPIOMUX_IN,
...
static struct msm_gpiomux_config factory_cable_detect_configs[] __initdata = {
	{
		.gpio = 53,
		.settings = {
			[GPIOMUX_SUSPENDED] = &factory_cable_detect_cfg,
		},
	},
};
...
void __init msm8226_init_gpiomux(void)
{
	...
	msm_gpiomux_install(msm_sensor_configs, ARRAY_SIZE(msm_sensor_configs));
		...
		msm_gpiomux_install(msm_sensor_configs_evt0, ARRAY_SIZE(msm_sensor_configs_evt0));
		msm_gpiomux_install(msm_sensor_configs_for_workaround_evt1_2, ARRAY_SIZE(msm_sensor_configs_for_workaround_evt1_2));
	...
	if (socinfo_get_oem_board_id() > EVT2){
		msm_gpiomux_install(msm_pn544_nfc_configs, ARRAY_SIZE(msm_pn544_nfc_configs));
	}else{
		msm_pn544_nfc_configs[0].gpio= PN544_ENABLE_GPIO;
		msm_pn544_nfc_configs[1].gpio= PN544_HOST_INT_GPIO;
		msm_gpiomux_install(msm_pn544_nfc_configs, ARRAY_SIZE(msm_pn544_nfc_configs));
	}
	#endif
	...
	msm_gpiomux_install(factory_cable_detect_configs, ARRAY_SIZE(factory_cable_detect_configs));
