...
static int gdsc_enable(struct regulator_dev *rdev)
{
	...
	if (sc->toggle_logic) {
		...
		if (ret) {
			dev_err(&rdev->dev, "%s QMCK: enable timed out: 0x%x\n",
				sc->rdesc.name, regval);
			udelay(TIMEOUT_US);
			regval = readl_relaxed(sc->gdscr);
			dev_err(&rdev->dev, "%s QMCK: final state: 0x%x (%d us after timeout)\n",
				sc->rdesc.name, regval, TIMEOUT_US);

			if (!strcmp(sc->rdesc.name, "gdsc_oxili_gx")){
				void * j_debug =  ioremap(0xFD8C4300, 0x8);

				errval = readl_relaxed(j_debug);
				dev_err(&rdev->dev, "%s QMCK: oxili misc 0x%08x  \n", sc->rdesc.name, errval);

				iounmap(j_debug);

				j_debug = ioremap(0xFC40024C, 0x8);
				errval = readl_relaxed(j_debug);
				dev_err(&rdev->dev, "%s QMCK: oxili AHB CBCR 0x%08x  \n", sc->rdesc.name, errval);
				iounmap(j_debug);
			}
			return ret;
		}
