...
static int dbg_led_brightness_set(void *data, u64 value)
{
        ...
        if (value == 9999) {
                led->fvs_mode_onoff_flag = 0; 
                printk("%s,%s exit ftd test mode\n", __func__,led->cdev.name);

        }else {
                led->fvs_mode_onoff_flag = 1;
        	if (value < LED_OFF || value > LED_FULL) {
                	dev_err(led->cdev.dev, "[LED] Invalid brightness value exceeds\n");
                	return -1;
        	}
                if (value >= 0 && value <= led->cdev.max_brightness)
                        led->cdev.brightness = value;
		else if (value > led->cdev.max_brightness)
...
static int __devinit qpnp_leds_probe(struct spmi_device *spmi)
{
	...
	bool regulator_probe = false;
        ...
        static struct dentry *temp_dent, *temp_file;
	...
	for_each_child_of_node(node, temp) {
		...
		if (rc) {
			...
			goto fail_id_check;
                ...
                temp_dent = debugfs_create_dir(led->cdev.name, dent);
       		if (temp_dent == NULL || IS_ERR(temp_dent)) {
            		pr_err("ERR: led dir: temp_dent=%p\n", temp_dent);
                	goto fail_id_check;
        	}

        	temp_file = debugfs_create_file("brightness", S_IRUGO | S_IWUGO,
                		temp_dent, led, &dbg_led_brightness_fops);


        	if (temp_file == NULL || IS_ERR(temp_file)) {
            		pr_err("ERR: led brightness: temp=%p\n", temp_file);
                	goto fail_id_check;
        	}
...
static int __init qpnp_led_init(void)
{
	...
	ret = spmi_driver_register(&qpnp_leds_driver);
