...
static int modem_logger_mmap(struct file *filp, struct vm_area_struct *vma)
{
	...
	if (!IS_PAGE_ALIGNED(length) || length > MSM_SHARED_RAM_SIZE) {
		printk(KERN_ERR "%s: Invalid length: %ld/%d.\n", __func__, length, MSM_SHARED_RAM_SIZE);
		return -EINVAL;
	}
	...
	addr = MSM8226_MSM_SHARED_RAM_PHYS;
	addr &= ~(PAGE_SIZE - 1);
