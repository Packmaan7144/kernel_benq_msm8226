		...
		if (!(ctrl & SDHCI_CTRL_TUNED_CLK)) {
			pr_info(DRIVER_NAME ": Tuning procedure"
				...
				" clock\n");
           		...
           		{
                             const char *mmc_name = mmc_hostname(host->mmc);
				if ( mmc_name[3] == '1' ) { 
                             		if ( ! host->mmc->default_caps )
                                     	host->mmc->default_caps = host->mmc->caps;
	                                    pr_err("%s: Try to lower clock rate\n",mmc_hostname(host->mmc));
	                                    pr_info("old cap is %lX", host->mmc->caps);
                                
                                	if ( host->mmc->caps & MMC_CAP_UHS_SDR104 ) {
                                     	pr_info("%s: Disable SDR_104\n",mmc_hostname(host->mmc));
	                                    ...
	                                    pr_info("%s: Disable SDR50\n",mmc_hostname(host->mmc));
       			                      host->mmc->caps &= ~MMC_CAP_UHS_SDR50;
                                
                                }else if ( host->mmc->caps & MMC_CAP_UHS_SDR25 ) {
	                                    pr_info("%s: Disable SDR25\n",mmc_hostname(host->mmc));
                                  		host->mmc->caps &= ~MMC_CAP_UHS_SDR25;
                            	   }
                                    
                            	}
                             pr_err("%s: Slow down the clock rate successfully\n",mmc_hostname(host->mmc));
			}
...
static void sdhci_tasklet_card(unsigned long param)
{
	...
	mmc_detect_change(host->mmc, msecs_to_jiffies(400));
}
...
static void sdhci_data_irq(struct sdhci_host *host, u32 intmask)
{
	...
	if (intmask & SDHCI_INT_DATA_TIMEOUT){
		pr_err("%s: DATA_TIMEOUT\n", mmc_hostname(host->mmc));
		host->data->error = -ETIMEDOUT;
	}else if (intmask & SDHCI_INT_DATA_END_BIT){
		pr_err("%s: DATA_END_BIT\n", mmc_hostname(host->mmc));
				...
				if (intmask & (SDHCI_INT_DATA_CRC | SDHCI_INT_DATA_TIMEOUT)){
					host->flags |= SDHCI_NEEDS_RETUNING;
						...
						if ( mmc_name[3] == '1' ) { 
		                             ...
		                             pr_info("%s:Lower the clock rate to avoid CRC error successfully\n",mmc_hostname(host->mmc));
					}
