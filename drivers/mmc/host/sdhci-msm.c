...
show_detect(struct device *dev, struct device_attribute *attr, char *buf)
{
	...
	ret = !gpio_get_value_cansleep(msm_host->pdata->status_gpio) ^
		!!(msm_host->mmc->caps2 & MMC_CAP2_CD_ACTIVE_HIGH);
out:
	return snprintf(buf, PAGE_SIZE, "%d\n", ret);
}
...
static int __devinit sdhci_msm_probe(struct platform_device *pdev)
{
	...
	if (!gpio_is_valid(msm_host->pdata->status_gpio)) {
		...
		msm_host->detect.show = show_detect;
		msm_host->detect.store = NULL;
		sysfs_attr_init(&msm_host->detect.attr);
		msm_host->detect.attr.name = "detect";
		msm_host->detect.attr.mode =  S_IRUGO;
		ret = device_create_file(&pdev->dev, &msm_host->detect);
		if (ret)
	...
	goto out;
remove_detect_file:
        device_remove_file(&pdev->dev, &msm_host->detect);
