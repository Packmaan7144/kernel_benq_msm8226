...
#define GPIO_FACTORY_CABLE_DETECT_PIN 53
#ifdef CONFIG_FACTORY_BUILD
#define GPIO_VOL_DOWN 107
#endif
...
static bool is_factory_cable = 0;
...
static int msm_hsusb_ldo_init(struct msm_otg *motg, int init)
{
	...
	if (init) {
		...
		if (IS_ERR(hsusb_3p3)) {
			pr_err("%s :[usb_otg] unable to get hsusb 3p3\n",__func__);
		...
		rc = regulator_set_voltage(hsusb_3p3, USB_PHY_3P3_VOL_MIN,
				USB_PHY_3P3_VOL_MAX);
		if (rc) {
			pr_err("%s :[usb_otg] unable to set voltage level for"
					"hsusb 3p3\n",__func__);
			return rc;
		}
		hsusb_1p8 = devm_regulator_get(motg->phy.dev, "HSUSB_1p8");
		if (IS_ERR(hsusb_1p8)) {
			pr_err("%s :[usb_otg] unable to get hsusb 1p8\n",__func__);
		...
		rc = regulator_set_voltage(hsusb_1p8, USB_PHY_1P8_VOL_MIN,
				USB_PHY_1P8_VOL_MAX);
		if (rc) {
			pr_err("%s :[usb_otg] unable to set voltage level "
					"for hsusb 1p8\n",__func__);
			goto put_1p8;
		}
...
static int msm_hsusb_config_vddcx(int high)
{
	...
	if (ret) {
		pr_err("%s :[usb_otg] unable to set the voltage for regulator "
			"HSUSB_VDDCX\n", __func__);
		return ret;
	}
	...
	enum msm_otg_phy_reg_mode mode)
{
	...
	if (IS_ERR(hsusb_1p8)) {
		pr_err("%s :[usb_otg] HSUSB_1p8 is not initialized\n",__func__);
		return -ENODEV;
	}

	if (IS_ERR(hsusb_3p3)) {
		pr_err("%s :[usb_otg] HSUSB_3p3 is not initialized\n",__func__);
		return -ENODEV;
	}
	...
	case USB_PHY_REG_ON:
		...
		if (ret < 0) {
			pr_err("%s :[usb_otg] Unable to set HPM of the regulator "
				"HSUSB_1p8\n", __func__);
			return ret;
		}
		...
		if (ret) {
			pr_err("%s :[usb_otg] unable to enable the hsusb 1p8\n",__func__);
		...
		if (ret) {
			pr_err("%s :[usb_otg] unable to enable the hsusb 3p3\n",__func__);
	...
	case USB_PHY_REG_OFF:
		...
		if (ret) {
			pr_err("%s :[usb_otg] unable to disable the hsusb 1p8\n",
				__func__);
			return ret;
		}
		...
		if (ret < 0)
			pr_err("%s :[usb_otg] Unable to set LPM of the regulator "
				"HSUSB_1p8\n", __func__);

		ret = regulator_disable(hsusb_3p3);
		if (ret) {
			pr_err("%s :[usb_otg] unable to disable the hsusb 3p3\n",
				 __func__);
			return ret;
		}
	...
	case USB_PHY_REG_LPM_ON:
		...
		if (ret < 0) {
			pr_err("%s :[usb_otg] Unable to set LPM of the regulator: HSUSB_1p8\n",
				__func__);
			return ret;
		}
		...
		if (ret < 0) {
			pr_err("%s :[usb_otg] Unable to set LPM of the regulator: HSUSB_3p3\n",
				__func__);
	...
	case USB_PHY_REG_LPM_OFF:
		ret = regulator_set_optimum_mode(hsusb_1p8,
				USB_PHY_1P8_HPM_LOAD);
		if (ret < 0) {
			pr_err("%s :[usb_otg] Unable to set HPM of the regulator: HSUSB_1p8\n",
				__func__);
			return ret;
		}
		...
		ret = regulator_set_optimum_mode(hsusb_3p3,
				USB_PHY_3P3_HPM_LOAD);
		if (ret < 0) {
			pr_err("%s :[usb_otg] Unable to set HPM of the regulator: HSUSB_3p3\n",
				__func__);
			regulator_set_optimum_mode(hsusb_1p8, USB_PHY_REG_ON);
			return ret;
		}
	...
	default:
		pr_err("%s :[usb_otg] Unsupported mode (%d).", __func__, mode);
		return -ENOTSUPP;
	}
...
static void msm_hsusb_mhl_switch_enable(struct msm_otg *motg, bool on)
{
	...
	if (on) {
		if (regulator_enable(mhl_usb_hs_switch))
			pr_err("%s :[usb_otg] unable to enable mhl_usb_hs_switch\n",__func__);
	} else {
...
psy_error:
	pr_err("%s :[usb_otg] power supply error when setting property\n",__func__);
	return -ENXIO;
}
...
static void msm_hsusb_vbus_power(struct msm_otg *motg, bool on)
{
	...
	if (!vbus_otg) {
		pr_err("%s :[usb_otg] vbus_otg is NULL.\n",__func__);
		return;
	}
	...
	if (on) {
		...
		if (ret) {
			pr_err("%s :[usb_otg] unable to enable vbus_otg\n",__func__);
			return;
		}
		vbus_is_on = true;
	} else {
		...
		if (ret) {
			pr_err("%s :[usb_otg] unable to disable vbus_otg\n",__func__);
			return;
		}
		msm_otg_notify_host_mode(motg, on);
		vbus_is_on = false;
...
static int msm_otg_set_host(struct usb_otg *otg, struct usb_bus *host)
{
	...
	if (!motg->pdata->vbus_power && host) {
		...
		if (IS_ERR(vbus_otg)) {
			pr_err("%s :[usb_otg] Unable to get vbus_otg\n",__func__);
					...
					struct usb_gadget *gadget)
{
	...
	if (motg->pdata->mode == USB_HOST) {
		pr_err("%s :[usb_otg] Peripheral mode is not supported\n",__func__);
		return -ENODEV;
	}
						...
						void (*callback)(int on))
{
	...
	if (!motg->pdata->mhl_enable) {
		pr_err("%s :[usb_otg] MHL feature not enabled\n",__func__);
		return -ENODEV;
	}

	if (motg->pdata->otg_control != OTG_PMIC_CONTROL ||
			!motg->pdata->pmic_id_irq) {
		pr_err("%s :[usb_otg] MHL can not be supported without PMIC Id\n",__func__);
		return -ENODEV;
	}

	if (!motg->pdata->mhl_dev_name) {
		pr_err("%s :[usb_otg] MHL device name does not exist.\n",__func__);
		return -ENODEV;
	}
	...
	if (ret)
		pr_debug("%s :[usb_otg] mhl_register_callback(%s) return error=%d\n",
						__func__,motg->pdata->mhl_dev_name, ret);
	else
...
static void msm_otg_id_timer_func(unsigned long data)
{
	...
	if (msm_chg_check_aca_intr(motg)) {
		pr_err("%s :[usb_otg] timer: aca work\n",__func__);
		queue_work(system_nrt_wq, &motg->sm_work);
	}
...
static void msm_chg_detect_work(struct work_struct *w)
{
	...
	case USB_CHG_STATE_DCD_DONE:
		...
		if (vout && !dm_vlgc) { /* VDAT_REF < DM < VLGC */
			...
			if (motg->chg_type == USB_SDP_CHARGER) {
				is_factory_cable = gpio_get_value_cansleep(GPIO_FACTORY_CABLE_DETECT_PIN);
			}
...
static void msm_otg_sm_work(struct work_struct *w)
{
	...
	case OTG_STATE_UNDEFINED:
		...
		if (!psy && legacy_power_supply) {
			...
			if (!psy)
				pr_err("%s :[usb_otg] couldn't get usb power supply\n",__func__);
		}
	...
	case OTG_STATE_B_IDLE:
		if (test_bit(MHL, &motg->inputs)) {
			...
			if (is_factory_cable) {
				is_factory_cable = 0;
				#ifdef CONFIG_FACTORY_BUILD
				if(!gpio_get_value_cansleep(GPIO_VOL_DOWN)) {
					kernel_power_off();
				}
				#endif
			}
			...
			{
			        if (!msm_otg_read_pmic_id_state(motg)) {
					pr_info("%s :[usb_otg] process missed ID intr and clear ID\n",__func__);
				        clear_bit(ID, &motg->inputs);
				        work = 1;
			        	break;
			        }
			}
			pr_debug("%s :[usb_otg] chg_work cancel to call autosuspend\n",__func__);
			pm_runtime_put_noidle(otg->phy->dev);
			/*
...
static void msm_otg_set_vbus_state(int online)
{
	...
	if (online) {
		...
		clear_bit(B_SESS_VLD, &motg->inputs);
		if (otg->gadget->enumeration)
			otg->gadget->enumeration = 0;
	}
