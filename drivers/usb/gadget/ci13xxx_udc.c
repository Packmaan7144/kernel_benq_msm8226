...
static int hw_device_reset(struct ci13xxx *udc)
{
	...
	while (delay_count--  && hw_cread(CAP_USBCMD, USBCMD_RST))
		udelay(10);
	if (delay_count < 0)
		pr_err("%s :[usb_udc] USB controller reset failed\n",__func__);
	...
	if (hw_cread(CAP_USBMODE, USBMODE_CM) != USBMODE_CM_DEVICE) {
		pr_err("%s :[usb_udc] cannot enter in device mode",__func__);
		pr_err("%s :[usb_udc] lpm = %i",__func__, hw_bank.lpm);
		return -ENODEV;
	}
...
static int hw_device_state(u32 dma)
{
	...
	if (dma) {
		if (gadget->streaming_enabled || !(udc->udc_driver->flags &
			...
			pr_debug("%s :[usb_udc] streaming mode is enabled. USBMODE:%x\n",
				__func__, hw_cread(CAP_USBMODE, ~0));
		} else {
			hw_cwrite(CAP_USBMODE, USBMODE_SDIS, USBMODE_SDIS);
			pr_debug("%s :[usb_udc] streaming mode is disabled. USBMODE:%x\n",
				__func__, hw_cread(CAP_USBMODE, ~0));
		}
		...
		if (udc->udc_driver->flags & CI13XXX_ENABLE_AHB2AHB_BYPASS) {
			...
			pr_debug("%s :[usb_udc] ByPass Mode is enabled. AHBMODE:%x\n",
		...
		if (udc->udc_driver->flags & CI13XXX_ENABLE_AHB2AHB_BYPASS) {
			...
			pr_debug("%s :[usb_udc] ByPass Mode is disabled. AHBMODE:%x\n",
					__func__, hw_aread(ABS_AHBMODE, ~0));
		}
	}
	return 0;
			    ...
			    const char *buf, size_t count)
{
	...
	if (sscanf(buf, "%u", &tty) != 1 || tty > 1) {
		pr_err("%s :[usb_udc] <1|0>: enable|disable console log\n",__func__);
		goto done;
	}
			    ...
			    const char *buf, size_t count)
{
	...
	if (sscanf(buf, "%u %u", &en, &bit) != 2 || en > 1) {
		pr_err("%s :[usb_udc] <1|0> <bit>: enable|disable interrupt",__func__);
		goto done;
	}
	...
	if (en) {
		if (hw_intr_force(bit))
			pr_err("%s :[usb_udc] invalid bit number\n",__func__);
		else
			isr_statistics.test++;
	} else {
			       ...
			       const char *buf, size_t count)
{
	...
	if (sscanf(buf, "%u", &mode) != 1) {
		pr_err("%s :[usb_udc] <mode>: set port test mode",__func__);
		goto done;
	}

	spin_lock_irqsave(udc->lock, flags);
	if (hw_port_test_set(mode))
		pr_err("%s :[usb_udc] invalid mode\n",__func__);
	spin_unlock_irqrestore(udc->lock, flags);
			      ...
			      struct device_attribute *attr, char *buf)
{
	...
	if (!dump) {
		pr_err("%s :[usb_udc] out of memory\n", __func__);
		return 0;
	}
			       ...
			       const char *buf, size_t count)
{
	...
	if (sscanf(buf, "%li %li", &addr, &data) != 2) {
		pr_err("%s :[usb_udc] <addr> <data>: write data to register address",__func__);
		goto done;
	}

	spin_lock_irqsave(udc->lock, flags);
	if (hw_register_write(addr, data))
		pr_err("%s :[usb_udc] invalid address range\n",__func__);
	spin_unlock_irqrestore(udc->lock, flags);
			       ...
			       const char *buf, size_t count)
{
	...
	if (sscanf(buf, "%u %u", &ep_num, &dir) != 2) {
		pr_err("%s :[usb_udc] <ep_num> <dir>: prime the ep",__func__);
		goto done;
	}
			       ...
			       const char *buf, size_t count)
{
	...
	if (sscanf(buf, "%u %u", &ep_num, &dir) != 2) {
		pr_err("%s :[usb_udc]<ep_num> <dir>: to print dtds",__func__);
		goto done;
	}
	...
	unsigned int ebi_apacket0, unsigned int ebi_apacket1)
{
	...
	ebi_err_data = kmalloc(sizeof(struct ci13xxx_ebi_err_data),
				 GFP_ATOMIC);
	if (!ebi_err_data) {
		pr_err("%s :[usb_udc] memory alloc failed for ebi_err_data\n", __func__);
		return;
	}
	...
	if (!ebi_err_data->ebi_err_entry) {
		kfree(ebi_err_data);
		pr_err("%s :[usb_udc] memory alloc failed for ebi_err_entry\n", __func__);
		return;
	}
	...
	for (i = 0; i < hw_ep_max; i++) {
		list_for_each(ptr, &udc->ci13xxx_ep[i].qh.queue) {
			...
			temp_dump->next = kmalloc(
					  ...
					  GFP_ATOMIC);
			if (!temp_dump->next) {
				pr_err("%s :[usb_udc] memory alloc failed\n", __func__);
				spin_unlock_irqrestore(udc->lock, flags);
				return;
			}
...
static void udc_remove(void)
{
	...
	retval = unregister_trace_usb_daytona_invalid_access(dump_usb_info,
									NULL);
	if (retval)
		pr_err("%s :[usb_udc] Unregistering trace failed\n",__func__);
