	...
	if (--dev->disable_depth == 0) {
		...
		list_for_each_entry(conf, &dev->configs, list_item) {
			err = usb_add_config(cdev, &conf->usb_config,
						android_bind_config);
			if (err < 0) {
				pr_err("[usb_gadget] usb_add_config failed : err: %d\n", err);
				return err;
			}
		}
...
static void adb_closed_callback(void)
{
	...
	if (!dev)
		pr_err("[usb_gadget] aadb_closed_callback: data->dev is NULL\n");
		...
		struct usb_configuration *c)
{
	...
	pr_info("[usb_gadget] MAC: %02X:%02X:%02X:%02X:%02X:%02X\n",
	...
	if (rndis->ethaddr[0])
		...
		ret = gether_setup_name(c->cdev->gadget, rndis->ethaddr,
								"rndis");
	if (ret) {
		pr_err("[usb_gadget] gether_setup failed\n");
		return ret;
	}
					...
					struct usb_composite_dev *cdev)
{
	...
	#elif defined (CONFIG_SHIP_BUILD)
	if(socinfo_get_factory_cable()) {
		config->fsg.luns[0].removable = 1;
	} else {
...
static int hidchg_function_init(struct android_usb_function *f,
					struct usb_composite_dev *cdev)
{
	hidg_plat_driver_probe(&usb_hidchg);
	return hid_bind(cdev);
}

static void hidchg_function_cleanup(struct android_usb_function *f)
{
	hid_cleanup();
}

static int hidchg_function_bind_config(struct android_usb_function *f,
						struct usb_configuration *c)
{
	return do_config(c);
}

static struct android_usb_function hidchg_function = {
	.name		= "hid",
	.init		= hidchg_function_init,
	.cleanup	= hidchg_function_cleanup,
	.bind_config	= hidchg_function_bind_config,
};
			    ...
			    const char *buff, size_t size)
{
	...
	if (enabled && !dev->enabled) {
		...
		if (err < 0) {
			pr_err("[usb_gadget] android_enable failed\n");
			dev->connected = 0;
			dev->enabled = false;
			mutex_unlock(&dev->mutex);
			return size;
		...
		pr_err("[usb_gadget] android_usb: already %s\n",
						...
						(struct android_dev *dev)
{
	...
	if (!conf) {
		pr_err("[usb_gadget] %s(): Failed to alloc memory for android conf\n",
			__func__);
...
static int usb_diag_update_pid_and_serial_num(u32 pid, const char *snum)
{
	...
	if (!diag_dload) {
		pr_err("[usb_gadget] : unable to update PID and serial_no\n");
		return -ENODEV;
	}
...
static int __devinit android_probe(struct platform_device *pdev)
{
	...
	if (pdev->dev.of_node) {
		...
		if (!pdata) {
			pr_err("[usb_gadget] unable to allocate platform data\n");
			return -ENOMEM;
		}
		...
		for (i = 0; i < len; i++) {
			...
			if (sizeof(name) > FUNC_NAME_LEN) {
				pr_err("[usb_gadget] Function name is bigger than allowed.\n");
				continue;
			}

			strlcpy(pdata->streaming_func[i], name,
				sizeof(pdata->streaming_func[i]));
			pr_info("[usb_gadget] %s:name of streaming function:%s\n",
				 __func__,pdata->streaming_func[i]);
		}
	...
	if (!android_dev) {
		pr_err("[usb_gadget]%s(): Failed to alloc memory for android_dev\n",
			__func__);
		ret = -ENOMEM;
		goto err_alloc;
	}
	...
	if (ret) {
		pr_err("[usb_gadget] %s(): android_create_device failed\n", __func__);
		goto err_dev;
	}

	ret = usb_composite_probe(&android_usb_driver, android_bind);
	if (ret) {
		pr_err("[usb_gadget] %s(): Failed to register android "
				 "composite driver\n", __func__);
		goto err_probe;
	}
