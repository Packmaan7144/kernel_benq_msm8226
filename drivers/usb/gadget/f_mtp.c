...
static uint32_t is_factory_cable_boot = 0;
...
fail:
	pr_err("%s :[usb_mtp] mtp_bind() could not allocate requests\n",__func__);
	return -1;
}
	...
	size_t count, loff_t *pos)
{
	...
	printk("Factory Build does not support MTP functions!\n");
	return -EINVAL;
	#elif defined (CONFIG_SHIP_BUILD)
	if (is_factory_cable_boot) {
		printk("Factory Cable Power On does not support MTP functions!\n");
		return -EINVAL;
	}
	#endif
				...
				const struct usb_ctrlrequest *ctrl)
{
	...
	if (value >= 0) {
		...
		if (rc < 0)
			pr_err("%s:[usb_mtp] response queue error\n",__func__);
	}
	return value;
}
...
mtp_function_bind(struct usb_configuration *c, struct usb_function *f)
{
	...
	pr_debug("%s :[usb_mtp] %s speed %s: IN/%s, OUT/%s\n",
			__func__,gadget_is_dualspeed(c->cdev->gadget) ? "dual" : "full",
...
static int mtp_setup(void)
{
	...
	is_factory_cable_boot = socinfo_get_factory_cable();
