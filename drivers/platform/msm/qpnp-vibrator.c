...
{
        struct qpnp_vib *vib = (struct qpnp_vib *)data;
...
static int dbg_vibrator_onoff_set(void *data, u64 value)
{
        ...
        if (value == 0) {
                spin_lock_irqsave(&vib->lock1, flags);
                vib->state= 0;
                spin_unlock_irqrestore(&vib->lock1, flags);
        } else if (value == 1) {
                spin_lock_irqsave(&vib->lock1, flags);
                vib->state = 1;
                spin_unlock_irqrestore(&vib->lock1, flags);
...
DEFINE_SIMPLE_ATTRIBUTE(vibrator_onoff_fops, dbg_vibrator_onoff_get,
                       dbg_vibrator_onoff_set, "%llu\n");
static void vibrator_create_debugfs_entries(struct qpnp_vib *vib)
{

        dent = debugfs_create_dir("vibrator", NULL);
        if (dent) {
...
static int __devinit qpnp_vibrator_probe(struct spmi_device *spmi)
{
	...
	vib_data=vib;
        vibrator_create_debugfs_entries(vib_data);
