...
static ssize_t panel_set_blue_light(struct device *dev, struct device_attribute *attr,
		const char *buf, size_t count)
{
	...
	if(count > 0) {
		mode = buf[0] - 0x30; 
		pr_debug("[BLUE-L], %s called\n", __func__);
		ctrl_pdata->set_blue_light_mode(ctrl_pdata, mode);
	} else {
		pr_debug("[BLUE-L], %s called, error blue light mode\n", __func__);
	}
	return count;
}

static ssize_t panel_read_blue_light(struct device *dev, struct device_attribute *attr,char *buf)
{
	struct platform_device *pdev = container_of(dev, struct platform_device, dev);
	struct mdss_dsi_ctrl_pdata *ctrl_pdata = platform_get_drvdata(pdev);
	return sprintf(buf, "%d\n", ctrl_pdata->current_cmd_set);
}
static DEVICE_ATTR(blue_light, 0644, panel_read_blue_light, panel_set_blue_light);

static int panel_create_sys_entries(struct platform_device *pdev)
{
	int ret;
	ret = device_create_file(&pdev->dev, &dev_attr_blue_light);
	return ret;
}
...
static int __devinit mdss_dsi_ctrl_probe(struct platform_device *pdev)
{
	...
	rc = panel_create_sys_entries(pdev);
	if (rc) {
