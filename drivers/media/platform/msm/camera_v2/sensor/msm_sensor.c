...
#define LVS1_WORKAROUND
#define GPIO_FOR_LVS1_DROP 62

#define MSG2(format, arg...)  printk(KERN_ERR "[CAM]" format "\n", ## arg)  

static struct msm_sensor_ctrl_t *s_ctrl_i2c;  
int front_cam_id = 0;
int back_cam_id = 0;
int front_cam_exposure_time = 30; 
int front_cam_iso_speed = 200;    
module_param(front_cam_id, int, 0444);
module_param(back_cam_id, int, 0444);
module_param(front_cam_exposure_time, int, 0444);
module_param(front_cam_iso_speed, int, 0444);

static uint8_t back_sensor_calied_production_module_id[16]; 
...
int32_t msm_sensor_power_up(struct msm_sensor_ctrl_t *s_ctrl)
{
	...
	MSG2("%s+   (%04X) (VIO count %d)", __func__,data->slave_info->sensor_id,sensor_vio_count);
	...
	for (index = 0; index < power_setting_array->size; index++) {
		...
		case SENSOR_GPIO:
			...
			{
				sensor_vio_count ++;
				if(sensor_vio_count != 1)
				{
					MSG2("%s, VIO count %d -> %d (bypass VIO on)",__func__,sensor_vio_count-1, sensor_vio_count);
					break;
				}
			}
	...
	#ifdef LVS1_WORKAROUND
			if(power_setting->seq_val == SENSOR_GPIO_VIO){
				pr_err("%s, apply pre-charge:%d\n", __func__, GPIO_FOR_LVS1_DROP);
				gpio_request(GPIO_FOR_LVS1_DROP, "cam_power_supply_for_lvs1_drop");
				gpio_set_value(GPIO_FOR_LVS1_DROP, 1);
				mdelay(2);
			}
	#endif
	...
	if( s_ctrl->func_tbl->sensor_read_otp != NULL)
	{
		s_ctrl->func_tbl->sensor_read_otp(s_ctrl);
		if ( data->slave_info->sensor_id == 0x8865 )
		{
			memset(back_sensor_calied_production_module_id, 0, sizeof(back_sensor_calied_production_module_id));
			memcpy(back_sensor_calied_production_module_id,
					&s_ctrl->otp_info->module_info.module_id[0],
					16*sizeof(uint8_t));
		}
	}
...
power_up_failed:
	...
	for (index--; index >= 0; index--) {
		...
		case SENSOR_GPIO:
			...
			{
				sensor_vio_count --;
				if(sensor_vio_count != 0)
				{
					MSG2("%s, VIO count %d -> %d (bypass VIO off)",__func__,sensor_vio_count+1, sensor_vio_count);
					break;
				}
			}
			...
			if(power_setting->seq_val == SENSOR_GPIO_VIO){
				mdelay(2);
				gpio_direction_input(GPIO_FOR_LVS1_DROP);
				gpio_free(GPIO_FOR_LVS1_DROP);
			}
	#endif
			break;
		case SENSOR_VREG:
...
int32_t msm_sensor_match_id(struct msm_sensor_ctrl_t *s_ctrl)
{
	...
	if (rc < 0) {
		...
		MSG2("%s, %s: read id failed", __func__, s_ctrl->sensordata->sensor_name);
		return rc;
	}

	CDBG("%s: read id: %x expected id %x:\n", __func__, chipid,
		s_ctrl->sensordata->slave_info->sensor_id);
	if (chipid != s_ctrl->sensordata->slave_info->sensor_id && chipid !=0x80) {
		
		MSG2("%s, chip id doesnot match",__func__);
		return -ENODEV;
	}

	
	{
		int type = s_ctrl->sensordata->sensor_init_params->position;
		if(type == FRONT_CAMERA_B)
		{
			if(front_cam_id != chipid)
			{
				MSG2("%s, Front Camera = %s (%04X)",__func__,s_ctrl->sensordata->sensor_name,chipid);
				front_cam_id = chipid;
			}
		}
		else if(type == BACK_CAMERA_B)
		{
			if(back_cam_id != chipid)
			{
				MSG2("%s, Back Camera = %s (%04X)",__func__,s_ctrl->sensordata->sensor_name,chipid);
				back_cam_id = chipid;
			}
		}
		else
		{
			MSG2("%s, Unknown Cam Id = %04X (%s) (type = %d)",__func__,chipid,s_ctrl->sensordata->sensor_name,type);
		}
	}
	...
	void __user *argp)
{
	...
	case CFG_GET_OTP_DATA:{
		if (s_ctrl->func_tbl->sensor_get_calib_params == NULL) {
			rc = -EFAULT;
		} else {
			struct otp_params otp_params;
			uint16_t sensor_id = s_ctrl->sensordata->slave_info->sensor_id;
			if(sensor_id == 0x8865)
			{
				otp_params.otp_data = kzalloc(sizeof(struct otp_struct_q), GFP_KERNEL);
				rc = s_ctrl->func_tbl->sensor_get_calib_params(
					s_ctrl,
					&otp_params);
				memcpy(cdata->cfg.otp_params.otp_data, otp_params.otp_data, sizeof(struct otp_struct_q));
				cdata->cfg.otp_params.size = otp_params.size;
				kfree(otp_params.otp_data);
			}
		}
		break;
	}

	case CFG_DO_LSC_CALIBRATION:{
		if(s_ctrl->func_tbl->sensor_write_lsc_calibration == NULL)
		{
			pr_err("%s, write_lsc_calibration is null\n", __func__);
		} else {
			rc = s_ctrl->func_tbl->sensor_write_lsc_calibration(s_ctrl);
		}
		break;
	}
...
int32_t msm_sensor_platform_probe(struct platform_device *pdev, void *data)
{
	...
	if(s_ctrl->sensordata->sensor_init_params->position == FRONT_CAMERA_B && front_cam_id)    
	{
		MSG2("%s, Bypass %s (%04X) (Front camera already Exist)",__func__,
			s_ctrl->sensordata->sensor_name,s_ctrl->sensordata->slave_info->sensor_id);
		return -ENODEV;
	}
	else if(s_ctrl->sensordata->sensor_init_params->position == BACK_CAMERA_B && back_cam_id) 
	{
		MSG2("%s, Bypass %s (%04X) (Back camera alrady Exist)",__func__,
			s_ctrl->sensordata->sensor_name,s_ctrl->sensordata->slave_info->sensor_id);
		return -ENODEV;
	}
...
static int get_value(void *data, u64 * val)
{
  struct msm_sensor_ctrl_t *s_ctrl = s_ctrl_i2c;
  int i, addr;
  uint16_t r_data;
  if(!s_ctrl_i2c)
  {
    printk(KERN_ERR "Camera disabled!\n");
    *val = 0;
    return 0;
  }
  switch((int)data)
  {
    case 1: 
      *val = i2c_addr;
      printk(KERN_ERR "R addr = 0x%04X\n",i2c_addr);
      break;
    case 2: 
      *val = i2c_count;
      printk(KERN_ERR "R count = %0d\n",i2c_count);
      break;
    case 3: 
      addr = i2c_addr;
      for(i=0; i<i2c_count; i++)
      {
        s_ctrl->sensor_i2c_client->i2c_func_tbl->i2c_read(s_ctrl->sensor_i2c_client,
          addr,&r_data,MSM_CAMERA_I2C_BYTE_DATA);
        if(i == 0)
          *val = r_data;
        printk(KERN_ERR "R 0x%04X = 0x%02X\n",addr,r_data);
        addr += 1;
      }
      break;
    case 4: 
      ...
      {
        s_ctrl->sensor_i2c_client->i2c_func_tbl->i2c_read(s_ctrl->sensor_i2c_client,
          addr,&r_data,MSM_CAMERA_I2C_WORD_DATA);
        if(i == 0)
          *val = r_data;
        printk(KERN_ERR "R 0x%04X = 0x%04X\n",addr,r_data);
        addr += 2;
      }
      break;
  }
...
static int set_value(void *data, u64 val)
{
  struct msm_sensor_ctrl_t *s_ctrl = s_ctrl_i2c;
  uint16_t w_data;
  if(!s_ctrl_i2c)
  ...
  switch((int)data)
  {
    case 1: 
      i2c_addr = (int)val;
      printk(KERN_ERR "W addr = 0x%04X\n",i2c_addr);
      break;
    case 2: 
      i2c_count = (int)val;
      if(i2c_count <= 1)        i2c_count = 1;
      else if(i2c_count > 128)  i2c_count = 128;
      printk(KERN_ERR "W count = %0d\n",i2c_count);
      break;
    case 3: 
      w_data = (uint16_t)val;
      s_ctrl->sensor_i2c_client->i2c_func_tbl->i2c_write(s_ctrl->sensor_i2c_client,
        i2c_addr,w_data,MSM_CAMERA_I2C_BYTE_DATA);
      printk(KERN_ERR "R 0x%04X = 0x%02X\n",i2c_addr,w_data);
      break;
    case 4: 
      w_data = (uint16_t)val;
      s_ctrl->sensor_i2c_client->i2c_func_tbl->i2c_write(s_ctrl->sensor_i2c_client,
        i2c_addr,w_data,MSM_CAMERA_I2C_WORD_DATA);
      printk(KERN_ERR "R 0x%04X = 0x%04X\n",i2c_addr,w_data);
      break;
  }
  return 0;
}
DEFINE_SIMPLE_ATTRIBUTE(i2c_fops, get_value, set_value, "0x%04llX\n");

static int back_cam_read_fuseID_read(struct file *file, __user char *buffer, size_t count,
                        loff_t *ppos)
{
	int ret = 0;
	char buf[64];
	if(*ppos == 0){
		ret = snprintf(buf, sizeof(buf), "%X%X%X%X%X%X%X%X%X%X%X%X%X%X%X%X",
			back_sensor_calied_production_module_id[0],
			back_sensor_calied_production_module_id[1],
			back_sensor_calied_production_module_id[2],
			back_sensor_calied_production_module_id[3],
			back_sensor_calied_production_module_id[4],
			back_sensor_calied_production_module_id[5],
			back_sensor_calied_production_module_id[6],
			back_sensor_calied_production_module_id[7],
			back_sensor_calied_production_module_id[8],
			back_sensor_calied_production_module_id[9],
			back_sensor_calied_production_module_id[10],
			back_sensor_calied_production_module_id[11],
			back_sensor_calied_production_module_id[12],
			back_sensor_calied_production_module_id[13],
			back_sensor_calied_production_module_id[14],
			back_sensor_calied_production_module_id[15] );
	}
	pr_err("%s, ret:%d, buf:%s\n", __func__, ret, buf);
	return simple_read_from_buffer(buffer, count, ppos, buf, ret);
}
static const struct file_operations ftd_back_cam_read_fuse_operations = {
	
	.read           = back_cam_read_fuseID_read,
};

static int __init msm_sensor_ftd_init(void)
{
	dent = debugfs_create_dir("msm_sensor_ftd", NULL);
	if(dent)
	{
		debugfs_create_file("back_cam_fuse_id", S_IRUGO, dent, NULL, &ftd_back_cam_read_fuse_operations);
		
		debugfs_create_file("a",  0644, dent, (void *)1, &i2c_fops);  
		debugfs_create_file("c",  0644, dent, (void *)2, &i2c_fops);  
		debugfs_create_file("d",  0644, dent, (void *)3, &i2c_fops);  
		debugfs_create_file("d2", 0644, dent, (void *)4, &i2c_fops);  
