	...
	void *data)
{
	...
	case MSM_CAMERA_LED_LOW:
		if (fctrl->torch_trigger) {
			...
			pr_err("%s, cfg->torch_current:%d\n", __func__, cfg->torch_current);
	...
	case MSM_CAMERA_LED_HIGH:
		...
		for (i = 0; i < fctrl->num_sources; i++)
			if (fctrl->flash_trigger[i]) {
				pr_err("%s, cfg->flash_current[%d]:%d\n", __func__, i, cfg->flash_current[i]);
...
int camera_flash_FTD_test(void*  parm, u64 mode)
{
	int i = 0, rc = 0;


      struct msm_led_flash_ctrl_t * fctrl = ( struct msm_led_flash_ctrl_t *)parm;
	...
	case 1: 
		if (fctrl->torch_trigger)
		{
			...
			pr_err("%s, MSM_CAMERA_LED_LOW, fctrl->torch_op_current is 0x%x \n", __func__, fctrl->torch_op_current);
		}
		else
		{
			pr_err("%s, fctrl->torch_trigger is null", __func__);
		}
       break;
	...
	default:
		pr_err("%s,invalid command, mode:%d\n", __func__,(int)mode);
	       rc = -EFAULT;
	}
...
DEFINE_SIMPLE_ATTRIBUTE(camera_flash_enable_fops, NULL ,camera_flash_FTD_test, "%llu\n");

int enable_flash_debugfs(struct msm_led_flash_ctrl_t * fctrl)
{
	int rc = 0;
	static struct dentry *temp_dent;

	dent = debugfs_create_dir("camera_flash", NULL);

	temp_dent = debugfs_create_dir("control", dent);

	if (temp_dent) {



		debugfs_create_file("mode", S_IRUGO | S_IWUGO, temp_dent, fctrl, &camera_flash_enable_fops);
