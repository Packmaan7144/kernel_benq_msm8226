...
static int set_battery_data(struct qpnp_bms_chip *chip)
{
	...
	} else if (chip->batt_type == BATT_T3_4V35_18400MAH) {   
		batt_data = &Qisda_A3i_T3_1542679_1840mAh_BMS_data; 
                pr_info("Qisda T3 battery profile is used.\n");
	} else {
...
static int32_t get_vcoin_cell(struct qpnp_bms_chip *chip)
{

	struct qpnp_vadc_result result;
	int rc;

	rc = qpnp_vadc_read(chip->vadc_dev, VCOIN, &result);

	if (rc) {
		pr_err("error reading vcoin channel = %d, rc = %d\n", VCOIN, rc);
		return rc;
	}
	pr_info("[QPNP-BMS] vcoin phy = %lld meas = 0x%llx\n", result.physical, result.measurement);
	pr_info("[QPNP-BMS] raw_code = 0x%x\n", result.adc_code);
	return result.physical;
}
...
static int get_calc(void *data, u64 * val)
{
	...
	*val = 0;
	...
	switch (param) {
		case GET_VCOIN:
			*val = (u32)get_vcoin_cell(the_chip);
			printk(KERN_INFO "[QPNP-BMS] GET_VCOIN=%lld\n", *val);
			break;
		case GET_BATT_ID:
			*val = (u32)read_battery_id(the_chip);
			printk(KERN_INFO "[QPNP-BMS] GET_BATT_ID=%lld\n", *val);
			break;
		default:
			printk(KERN_INFO "[QPNP-BMS] error command.\n");
			ret = -EINVAL;
	}
	return ret;
}
...
static void create_debugfs_entries(struct qpnp_bms_chip *chip)
{
	pr_info("[QPNP-BMS] create qpnp_bms dentry.\n");

	chip->dent = debugfs_create_dir("qpnp-bms", NULL);
	if (IS_ERR(chip->dent)) {
                pr_err("pmic bms couldnt create debugfs dir\n");
                return;
        }

	debugfs_create_file("vcoin", 0644, chip->dent, (void *)GET_VCOIN, &calc_fops);
	debugfs_create_file("batt_id", 0644, chip->dent, (void *)GET_BATT_ID, &calc_fops);
}
